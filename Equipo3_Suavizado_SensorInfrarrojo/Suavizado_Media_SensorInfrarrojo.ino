...
extern double infrarrojo_sensor [tamano_se];
...
void loop() {
  moving_average((double *)&infrarrojo_sensor[0], (double *)&salida_senial[0], (int)tamano_se,9 ); //ventana de 11 el tiempo de cada una
  SNRsensor();
  plot_senial();
  delay(100);
}
...
void SNRsensor(){
  ...
  for(int i=0;i<tamano_se;i++){
    snr[i]=pow(infrarrojo_sensor[i],2);
    snr2[i]=pow((infrarrojo_sensor[i]-salida_senial[i]),2);
...
void plot_senial(void){       //el segundo void no necesita ningun parametro de entrada
  ...
  for(i=0;i<tamano_se;i++){
    Serial.print(infrarrojo_sensor[i]+600);
    Serial.print(",");
    Serial.print(infrarrojo_sensor[i]+10);
    Serial.print(",");
